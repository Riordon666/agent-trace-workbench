...
- The published implementation and release-infrastructure head is `2ab68f9`; this audit update records the verified state that followed it.
- Current development version: `0.3.0-dev.0`.
- [Remote CI run `31573392968`](https://github.com/Riordon666/agent-trace-workbench/actions/runs/31573392968) passed all 9 Windows/macOS/Linux source jobs on Node 20/22/24 and all 3 isolated package-smoke jobs on Node 20 using SHA-pinned official Actions.
- Package smoke ran the prepack gate with all 86 tests, then verified a clean tarball install, offline `atw` bin, public Adapter SDK, Doctor, localhost startup, per-user data storage, status API, unmodified install directory, and clean shutdown.
...
- The tag-triggered Release workflow is published on `main` with stable metadata gates, npm OIDC/provenance, registry/install verification, and npm-before-public-GitHub-Release sequencing. It is not listed as a default-branch workflow until an administrator switches the repository default to `main`.
...
| 3 | Fix CI badge | Verified on remote | README badge targets `branch=main`; workflow runs on push/PR; run `31573392968` passed all 12 jobs. | None beyond keeping CI healthy. |
| 4 | Unify version at `v0.2.0` | Verified for historical preview | Immutable `v0.2.0` tag and matching GitHub Release exist. The reproducible package audit found a known Ubuntu Doctor defect, so this version must remain unpublished on npm. Current `main` correctly uses `0.3.0-dev.0`. | Preserve the GitHub preview; make a later cross-platform verified stable version the first npm release. |
...
| 6 | Publish npm package | Not achieved; automation ready | Registry returns `E404`; `npm whoami` returns `ENEEDAUTH`. `v0.2.0` is excluded by its package audit; future stable tags have an OIDC/provenance workflow and tested metadata gate. | Administrator switches default branch and configures the GitHub Environment; npm owner claims the package with a later verified stable release, then configures the exact Trusted Publisher and verifies installation. |
...
| Area | Current result |
|---|---|
| Distribution readiness | Source/tarball behavior and future tag publication automation are verified; registry distribution remains unavailable until initial npm owner publication and Trusted Publisher configuration. |
| Cross-platform quality | Public CI run `31573392968` passed Windows/macOS/Linux source checks on Node 20/22/24 plus isolated package-smoke on all three operating systems. |
