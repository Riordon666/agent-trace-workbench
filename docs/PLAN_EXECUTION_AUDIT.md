...
| 6 | Publish npm package | Not achieved; automation ready | Registry returns `E404`; `npm whoami` returns `ENEEDAUTH`. Future stable tags have an OIDC/provenance release workflow and tested metadata gate. | npm owner first publishes immutable `v0.2.0`, administrator switches default branch and configures the exact GitHub Environment/Trusted Publisher, then verifies clean installation. |
